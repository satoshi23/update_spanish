...
function c2266498.initial_effect(c)
	...
	e1:SetCost(c2266498.cost)
	e1:SetTarget(c2266498.target)
	e1:SetOperation(c2266498.activate)
	c:RegisterEffect(e1)
end
function c2266498.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	e:SetLabel(100)
...
function c2266498.cfilter(c,e,tp,ft,bs)
	if c:IsSetCard(0x106) and c:IsType(TYPE_RITUAL) 
		...
		local lv=c:GetLevel()
		local g=Duel.GetMatchingGroup(c2266498.filter,tp,LOCATION_REMOVED,0,nil,e,tp)
		return g:GetCount()>0 and g:IsExists(c2266498.spfilter,1,nil,lv,ft,g,Group.CreateGroup(),bs)
...
function c2266498.spfilter(c,lv,ft,g,sg,bs)
	...
	return nlv>=0 and (nlv==0 or (not bs and g2:IsExists(c2266498.spfilter,1,nil,nlv,ft-1,g2,sg2,bs)))
end
function c2266498.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then
		...
		return Duel.IsExistingMatchingCard(c2266498.cfilter,tp,LOCATION_HAND,0,1,nil,e,tp,ft,bs)
	end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_CONFIRM)
	local tc=Duel.SelectMatchingCard(tp,c2266498.cfilter,tp,LOCATION_HAND,0,1,1,nil,e,tp,ft,bs):GetFirst()
...
function c2266498.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	while g:GetCount()>0 and ft>0 and lv>0 do
		...
		local sc=g:FilterSelect(tp,c2266498.spfilter,1,1,nil,lv,ft,g,sg,bs):GetFirst()
		sg:AddCard(sc)
